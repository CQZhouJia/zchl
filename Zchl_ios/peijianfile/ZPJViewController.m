...
#import "ZPJViewController.h"
#import "peileftCell.h"
#import "peirightCell.h"
#import "peijiandetaiViewController.h"

#define leftWith 80

@interface ZPJViewController ()<UITableViewDelegate,UITableViewDataSource,UICollectionViewDelegate,UICollectionViewDataSource>
...
@property (nonatomic,strong)NSMutableArray * leftMuArray;
@property (nonatomic,strong)NSMutableArray * leftFlagArray;
...
@implementation ZPJViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.view.backgroundColor = [UIColor whiteColor];
    _myArray = @[@"peicoll1",@"peicoll2"];
    _leftArray = @[@"底盘件",@"液压件",@"结构件",@"管虑件",@"电器件"];
    [self.navigationController setNavigationBarHidden:YES];
    [self setNav];
    [self initMainView];
    
//    _leftMuArray = [NSMutableArray array];
...
-(void)setNav{
    ...
    UIColor * colorOne = Color_RGBA(1, 249, 226, 1.0);
    UIColor * colorTwo = Color_RGBA(63, 143, 234, 1.0);
...
-(void)initData{
    ...
    NSDictionary * param = @{@"flag":@"1"};
    [ZTHttpTool postWithUrl:@"Common/CategoryQuery" param:param success:^(id responseObj) {
        //
        NSLog(@"%@",responseObj);
        NSLog(@"%@",responseObj[@"Data"][0][@"title"]);
        _leftMuArray = [[NSMutableArray alloc] initWithArray:responseObj[@"Data"]];
        NSLog(@"%zd",_leftMuArray.count);
        for (NSInteger i = 0; i < _leftMuArray.count; i ++) {
...
-(void)initMainView{
    ...
    _lefttableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 64 + 42, leftWith, HEIGHT - 64 - 49 - 42) style:UITableViewStylePlain];
    ...
    UIView * lineView = [[UIView alloc] initWithFrame:CGRectMake(leftWith, 64, 1, HEIGHT - 64 - 49)];
    lineView.backgroundColor = Color_RGBA(215, 215, 215, 1.0);
    [self.view addSubview:lineView];
    
    UICollectionViewFlowLayout * flowLayout = [[UICollectionViewFlowLayout alloc] init];
    flowLayout.scrollDirection = UICollectionViewScrollDirectionVertical;
    _rightcollectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(leftWith + 1, 64, WIDTH - leftWith - 1, HEIGHT - 64 - 49) collectionViewLayout:flowLayout];
...
//    cell.namelabel.text = _leftArray[indexPath.row];
    ...
    NSDictionary * dict = _leftMuArray[indexPath.row];
...
//    cell.logoImage.image = [UIImage imageNamed:_myArray[indexPath.row%2]];
    
    
    NSDictionary * dic = _myMuArray[indexPath.row];
    [cell.logoImage sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",dic[@"staticHttpUrl"],dic[@"img"]]] placeholderImage:[UIImage imageNamed:DefaultPictue]];
...
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    peijiandetaiViewController * page = [[peijiandetaiViewController alloc] init];
    page.proId = dic[@"proId"];
